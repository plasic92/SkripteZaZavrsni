...
$customEventObject = foreach($event in $event11){
    ...
    #cijela putanja foldera isčitana sa eventloga
    $ObjectName = $event.Properties[5].Value
    if([string]::IsNullOrEmpty(([System.IO.FileInfo]$ObjectName).Extension)){
        try{
            #ne pravim custom object nego uzimam gotov objekt od access controlla i dodajem odgovarajuće props 
            #get-acl je dosata sporiji od GetAccessControl() metode
...
while ($events){
    Write-customEvents -event11 $events
