...
$customEventObject = foreach($event in $event4670){
    if($event.Properties[6].Value -match 'c:\\share'){
        [PSCustomObject]@{
            ...
            OldSd = $event.Properties[8].Value
            NewSd = $event.Properties[9].Value
            ProcessName = $event.Properties[11].Value
        }
    }
}

$putanja = 'C:\Users\Administrator.DOMENAZAPIS\Desktop\skripte\logSecurityChange.csv'
$header = '"TimeReceived"*"TimeCreated"*"SID"*"UserName"*"Domain"*"SubjectLogonId"*"ObjectName"*"OldSd"*"NewSd"*"ProcessName"'
